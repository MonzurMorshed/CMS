                ...
                {{-- @foreach ($services as $service) --}}
                    <tr class="border-t">
                        ...
                        <td class="p-2 flex space-x-2">
                            ...
                            <!-- Delete Form -->
                            <form action="{{ url('cms-admin/service/delete/'. $service?->id) }}" method="POST" class="inline" onsubmit="return confirm('Delete this service?')">
                                @csrf @method('DELETE')
                    ...
                    <div id="editServiceModal{{ $service?->id }}" class="modal hidden">
                        <div class="modal-box">
                            ...
                            <form action="{{url('cms-admin/service/update')}}" method="POST" enctype="multipart/form-data">
                                @csrf
                                <input type="text" name="title" value="{{ $service?->title }}" class="w-full border p-2 mb-2" placeholder="Service Title" required>
                                <input type="text" name="description"  value="{{ $service?->description }}" class="w-full border p-2 mb-2" placeholder="Service Description" required>
                                <input type="file" name="image" value="{{ $service?->image }}" class="w-full border p-2 mb-2" required>
